            ...
            <table id="tOTList" class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr {{--php($query = "") @foreach($singleuser->log as $logs) @if(strpos($logs->message,"Query")!==false) @php($query = $logs->message) @endif @endforeach @if($singleuser->status=="Query")title = "{{str_replace('")', '', str_replace('Query ("', '', $query))}}"@endif--}}>
                        ...
                        <td @foreach($singleuser->log as $logs) @if(strpos($logs->message,"Query")!==false) @php($query = $logs->message) @endif @endforeach @if(($singleuser->status=="Query (Complete)")||($singleuser->status=="Query (Incomplete)"))title = "{{str_replace('")', '', str_replace('Query ("', '', $query))}}"@endif> @if(($singleuser->status=="Draft (Complete)")||($singleuser->status=="Draft (Incomplete)"))Draft <p style="color: red">Due: {{$singleuser->date_expiry}}</p> @elseif(($singleuser->status=="Query (Complete)")||($singleuser->status=="Query (Incomplete)")) @php($query = "") <p >Query</p>@else {{ $singleuser->status}} @endif</td>
                        <td>
