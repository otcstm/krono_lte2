    ...
    public function up()
    {
        ...
        VIEW `v_ot_rpt2` AS
            SELECT
                ...
                `ps`.`region` AS `u_region`,
                `st`.`item3` AS `status_descr`,
                `st2`.`item3` AS `day_type_descr`,
                `otd`.`id` AS `otd_id`,
                `otd`.`start_time` AS `otd_start_time`,
                `otd`.`end_time` AS `otd_end_time`,
                `otd`.`is_manual` AS `otd_is_manual`,
            ...
            FROM
                ...
                LEFT JOIN `overtime_details` `otd` ON (`otd`.`ot_id` = `ot`.`id`))
        ");
    }
